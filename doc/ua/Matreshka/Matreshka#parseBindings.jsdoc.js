...
@desc Починаючи з версії 1.1, Seemple.js включає в себе простий DOM парсер, що обробляє синтаксичні конструкції, укладені в подвійні фігурні дужки. Метод ``parseBindings`` першим аргументом приймає HTML строку, DOM вузол або селектор, відповідний DOM вузлу.
...
> Якщо фігурні дужки не влаштовують, поміняйте їх на щось інше, використовуючи  {@link Seemple.parserBrackets}

#### Навіщо потрібен такий метод?
У разі, якщо ви розробляєте велику форму зі стандартними HTML5 полями, метод допоможе зберегти час на оголошення численних прив'язок. Крім цього, ``parseBindings`` корисний у разі створення дуже простих колекцій, які не потребують реалізації складної {@link Seemple.Array#Model моделі}.


@param {string|node|$nodes} node - HTML строка, селектор, DOM вузол або колекція DOM вузлів HTML
@param {eventOptions} [eventOptions] - Об'єкт події, який буде переданий у всі внутрішні виклики методу {@link Seemple#bindNode}
