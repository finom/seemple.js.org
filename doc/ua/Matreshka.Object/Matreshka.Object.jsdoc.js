...
@importance 1
@classdesc ``Matreshka.Object`` - клас, який відповідає за дані вида ключ-значення. Його завданням є відділення службових властивостей від даних, які можна передати на сервер або зберегти в локальному сховищі. Клас успадковується від класу {@link Matreshka} і включає всі його властивості та методи.
...
Для того, щоб відокремити такі властивості від інших, можна скористатися методом {@link Matreshka.Object#addDataKeys}.
```js
...
Якщо ви заздалегідь не знаєте, які властивості є даними, можна завжди використовувати метод {@link Matreshka.Object#setData}, який не тільки оголошує властивості, що відповідають за дані, але і відразу встановлює значення.
```js
this.setData({
	a: 1,
	b: 2
...
Після того, як додаток отримав інформацію про те, що є даними, екземпляр {@link Matreshka.Object} можна конвертувати в звичайний об'єкт методом {@link Matreshka.Object#toJSON} і передати на сервер або зберегти в локальній БД (наприклад, в ``localStorage``).
```js
