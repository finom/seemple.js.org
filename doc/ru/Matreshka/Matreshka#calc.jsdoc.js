...
> Обратите внимание, что у метода есть {@link Seemple.calc статичный аналог}, который работает в точности так же, но принимает любой объект в качестве первого аргумента, cдвигая остальные аргументы вправо.
```js
...
В случае, если нужно изменить значение свойства-источника и сделать это так, чтоб целевое свойство не было вычислено заново, используйте метод {@link Seemple#set} с флагом ``skipCalc`` равным ``true``.
...
@param {eventOptions} [eventOptions] - Объект, в который можно передать какие-нибудь данные для обработчика события, слушающего изменения ``target`` или специальные флаги (см. описание метода)
...
@variation batch
@since 2.0
@summary Дополнительная возможность метода {@link Seemple#calc}, позволяющая объявить несколько вычислимых свойств одним вызовом
...
``source`` может принимать любой вид {@link Seemple#calc описаный выше} (строка, массив строк, объект со свойствами ``key`` и ``object``, массив объектов).

@param {array} batch - Объект содержащий информацию о вычислимых свойствах
@param {eventOptions} [commonEventOptions] - Общие свойства для объектов события
