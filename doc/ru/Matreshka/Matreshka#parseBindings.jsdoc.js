...
@desc Начиная с версии 1.1, Seemple.js включает в себя простой DOM парсер, обрабатывающий синтаксические конструкции, заключенные в двойные фигурные скобки. Метод ``parseBindings`` первым аргументом принимает HTML строку, DOM узел или селектор, соответствующий DOM узлу.
...
> Если фигурные скобки не устраивают, поменяйте их на что-то другое, используя {@link Seemple.parserBrackets}

#### Зачем нужен такой метод?
В случае, если вы разрабатываете большую форму со стандартными HTML5 полями, метод поможет сохранить время на объявление многочисленных привязок. Кроме этого, ``parseBindings`` полезен в случае создания очень простой коллекции, не требующей реализации сложной {@link Seemple.Array#Model модели}.

@param {string|node|$nodes} node - HTML строка, селектор, DOM узел или коллекция DOM узлов
@param {eventOptions} [eventOptions] - Объект события, который будет передан во все вызовы метода {@link Seemple#bindNode}
