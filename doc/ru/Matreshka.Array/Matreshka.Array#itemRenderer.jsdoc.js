...
@desc Свойство ``itemRenderer`` - это переопределяемое (виртуальное) свойство, которое позволяет рендерить элементы массива без дополнительного кода. При вставке нового элемента в массив, автоматически создается HTML узел. Этот узел становится песочницей (см. {@link Seemple#bindNode}) (это поведение можно отменить, см. ниже) для вставленного элемента и встраивается в HTML контейнер, определенный в массиве.

> Для краткости, в примерах к этой статье будет использоваться синтаксис class fields.

#### Куда вставляется созданный элемент?
Для того, чтобы определить место, в которое будут вставляться отрисованные HTML узлы, нужно определить **контейнер**. Для этого следует объявить HTML песочницу для массива либо связать специальный ключ ``container`` с HTML контейнером. Подробнее о привязках и песочнице см. {@link Seemple#bindNode}.
...
На случай, если вы выносите шаблоны для элементов на HTML страницу, ``itemRenderer`` поддерживает селектор в качестве значения. В этом случае, {@link Seemple.Array} будет искать HTML элемент в DOM дереве и извлечет ``innerHTML`` найденого элемента. В случае, если элемент не найден, бросается исключение.
...
Иногда удобно объявлять рендерер внутри класса {@link Seemple.Array#Model}, а не на уровне коллекции. Свойство ``renderer`` перекрывает значение ``itemRenderer``, если оно задано для элемента коллекции.
```js
...
Не забывайте, что в Seemple.js реализована возможность отлова делегированных событий. Т. е. сам массив может отловить событие рендеринга элемента, используя имя события ``*@render`` (см. документацию к {@link #typedef-eventNames}).
```js
this.on('*@render', () => {
...
#### Отмена рендеринга
Как видно выше, если у дочернего элемента задано свойство ``render``, ``Seemple.Array`` попробует его отрисовать. Для того, чтоб полностью отменить рендеринг для массива, присвойте свойству массива ``renderIfPossible`` значение ``false``.
```js
class MyArray extends Seemple.Array {
...
Для отрисовки только тех объектов, которые еще не были отрисованы, воспользуйтесь методом {@link Seemple#set} с флагом ``forceRerender`` со значением  ``false``
```js
this.set('itemRenderer', renderer, {
	forceRerender: false
});
```
Такая необходимость может возникнуть тогда, когда вы используете серверный пререндеринг (см. {@link Seemple.Array#restore}), а шаблон подгружается динамически.
```js
class MyArray extends Seemple.Array {
	constructor() {
		super()
...
Объект, входящий в коллекцию, не обязательно должен быть экземпляром ``Seemple``, можно рендерить любой объект. Байндинги для таких объектов можно объявить используя статичный метод {@link Seemple.bindNode}.
