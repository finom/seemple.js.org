...
@inherits Matreshka
@importance 1
@classdesc Класс ``Matreshka.Array`` служит коллекцией во фреймворке Matreshka.js. Он наследуется от класса {@link Matreshka} и включает все его свойства и методы. Кроме этого, ``Matreshka.Array`` имеет все методы, которые есть у обычного массива, тем самым упрощая изучение его возможностей.
...
По причине того, что методы работают так же, как и оригинальные (с небольшими исключениями), они не приведены в этой документации по отдельности, а выведены в раздел {@link Matreshka.Array#METHOD}.
```js
this.push(1, 2, 3);
this.pop();
```
