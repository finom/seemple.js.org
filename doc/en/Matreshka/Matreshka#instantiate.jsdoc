...
@desc
> Note that the method has {@link Matreshka.instantiate static alternative}.
The method creates and fixes an instance of a class as given property value. When you try to override the property, instead of actual overriding, the instance is updated. Thus, we achieve integrity of an application, made on with Matreshka.

> The method is an addon over {@link Matreshka#mediate} and it overrides a mediator.
...
> It looks easy in practice: you create an ordinary class which almost always receives some data to be handled (for example, to use them in {@link Matreshka.Object#setData} method).

While attempting to assign another value (an object or an array) to the property, the inner mechanism of ``instantiate`` method performs the following instead of actual assignment:
- If ``updateCallback`` function is given, the method launches it with two arguments: current property value and the data which code is trying to assign.
- If given class is inherited from {@link Matreshka.Object}, the instance is updated with new data using {@link Matreshka.Object#setData} method.
- If given class is inherited from {@link Matreshka.Array}, the instance is updated with new data using {@link Matreshka.Array#recreate} method.
...
> A feature of this method is the absence of limitations on the class source. Any function-constructor that can be initialized using ``new`` operator and not only {@link Matreshka}'s successors can act as a class.

@param {string|array} key - Key or an array of keys
...
class MyParentClass extends Matreshka {
	constructor() {
...
this.instantiate('x', MyClass, () => {});
@example <caption>In case if your class doesn't support the use of ``new`` operator, use @link Matreshka#mediate} method instead of ``instantiate``.</caption>
...
@variation 2
@since 1.1
@summary Additional syntax for {@link Matreshka#instantiate}, which accepts key-class object as the first argument.
